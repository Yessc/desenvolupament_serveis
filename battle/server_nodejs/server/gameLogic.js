...
const WAITING_DURATION_MS = 10 * 1000;
...
const PLANT_DECELERATION_PER_SECOND = 75;
...
class GameLogic {
    constructor() {
        ...
        const spritesArray = LEVEL.sprites || (LEVEL.levels && LEVEL.levels[0] && LEVEL.levels[0].sprites) || [];

        console.log(`Sprites encontrados en el JSON: ${spritesArray.length}`);

        // 2. FILTRAR GEMAS
        // En gameLogic.js
        this.gems = spritesArray
            ...
            .map((s, index) => ({
                ...
                height: s.height || 14,
                type: "Gema",
                animationId: s.animationId, // <--- ESTO ES VITAL
                visible: true
            }));

        console.log(`Gemas cargadas: ${this.gems.length}`);

        // 1. CARGA ÚNICA DE NIVEL (Usamos la constante LEVEL que ya tienes arriba)
        this.levelData = LEVEL;
        this.worldWidth = LEVEL.worldWidth;
        this.worldHeight = LEVEL.worldHeight;
        // Dentro del constructor
        this.gemTemplates = buildGemTemplateMap(LEVEL.sprites);
        console.log("Estructura de LEVEL:", Object.keys(LEVEL));
        console.log("gentemplates:", this.gemTemplates);
        this.spawnGems();

        // 2. CONFIGURACIÓN DE COLISIONES (CHOQUE TOTAL)
        // Buscamos los nombres exactos de tu JSON para que bloqueen el paso
        const muros = classifyZoneIndices(['Muros', 'muros', 'wall', 'mur'], LEVEL.zones) || [];
        const plantas = classifyZoneIndices(['Plantas', 'plantas', 'plant', 'Segura'], LEVEL.zones) || [];
        const objetos = classifyZoneIndices(['Objetos', 'Category 5'], LEVEL.zones) || [];

        // FUSIONAMOS TODO en wallZoneIndices para que nadie pueda pasar por encima
        this.wallZoneIndices = classifyZoneIndices(['mur', 'wall', 'muros', 'Muros', 'Plantas', 'Objetos'], LEVEL.zones);
        console.log(`Zonas sólidas cargadas: ${this.wallZoneIndices.length}`);
        // Listas vacías para evitar errores de "is not iterable" en otras partes del código
        this.plantZoneIndices = [];
        this.sandZoneIndices = [];
        this.iceZoneIndices = [];
        console.log(LEVEL.zones[0]);

        // 3. ESTADOS DE CAPAS Y ZONAS
        ...
        // Extraemos los tanques de los sprites
        this.tanks = (LEVEL.sprites || [])
            .filter(s => s.type === "Tank_r" || s.type === "Tank_l")
            .map(s => ({
                x: s.x,
                y: s.y,
                width: s.width || 64,
        ...
        // 6. INICIALIZAR GEMAS EN SU LUGAR
        // Usamos spawnGems pero asegurándonos de que use las celdas fijas
    ...
    addClient(id) {
        ...
        const player = {
            ...
            // --- CONTROL DE VIDA ---
            health: 100.0,      // Empieza con 100 de vida
            maxHealth: 100.0,   // Guardamos el tope para las curaciones
            ...
            switch (obj.type) {
                ...
                case 'shoot':
                    if (this.phase === 'playing') {
                        // Primero verificamos que el que dispara no esté muerto
                        if (player.isDead) return false;

                        player.isShooting = true;
                        const bulletRange = 150;
                        const hitBoxSize = 50;

                        for (let [otherId, target] of this.players) {
                            if (otherId === id) continue; // No dispararse a uno mismo
                            if (target.isDead) continue;  // No disparar a alguien ya muerto
                            ...
                            if (distance < bulletRange) {
                                if (player.facing === 'right' && dx > 0 && Math.abs(dy) < hitBoxSize) hit = true;
                                if (player.facing === 'left' && dx < 0 && Math.abs(dy) < hitBoxSize) hit = true;
                                if (player.facing === 'up' && dy < 0 && Math.abs(dx) < hitBoxSize) hit = true;
                                if (player.facing === 'down' && dy > 0 && Math.abs(dx) < hitBoxSize) hit = true;
                            }

                            // --- AQUÍ VA TU BLOQUE ---
                            if (hit) {
                                // 1. El atacante gana puntos
                                player.score += 20;

                                // 2. La víctima pierde vida (en lugar de solo puntos)
                                const damage = 50; // Cuánta vida quita cada bala
                                target.health -= damage;

                                console.log(`¡ZAS! ${player.name} golpeó a ${target.name}. Vida restante: ${target.health}`);

                                // 3. ¿Ha muerto?
                                if (target.health <= 0) {
                                    target.isDead = true;
                                    target.health = 0;
                                    console.log(`¡K.O! ${target.name} ha sido eliminado.`);

                                    // Penalización por morir (opcional)
                                    target.score = Math.max(0, target.score - 10);

                                    // Respawn: Reaparece en 2 segundos
                                    setTimeout(() => {
                                        target.isDead = false;
                                        target.health = target.maxHealth; // Recupera la vida

                                        // Lo mandamos a un punto de inicio aleatorio
                                        const respawnPos = this.getSpawnPosition(Math.floor(Math.random() * 4));
                            ...
                            // 4. Programamos la reaparición (Respawn)
                            setTimeout(() => {
                                target.isDead = false;
                                this.initialStateDirty = true;
                            }, 2000);
                        ...
                        // Animación de disparo: volver a false tras 400ms
                        setTimeout(() => {
    ...
    updateGame(fps) {
        ...
        for (const player of this.players.values()) {
            ...
            const onPlants = this.playerOverlapsAnyZone(player, this.plantZoneIndices);
            const speedMultiplier = onPlants ? SAND_SPEED_MULTIPLIER : 1;
            ...
            const acceleration = NORMAL_ACCELERATION_PER_SECOND;
            const deceleration = NORMAL_DECELERATION_PER_SECOND;
            ...
            // En updateGame, cambia el bucle de tanques por este:
            for (const tank of this.tanks) {
                if (this.checkCollision(player, tank)) {
    ...
    // Añade esto dentro de la clase GameLogic
    checkCollision(objA, objB) {
        const rectA = rectAt(objA.x, objA.y, objA.width, objA.height);
        const rectB = rectAt(objB.x, objB.y, objB.width, objB.height);
    ...
    spawnGems() {
        ...
        const todosLosSprites = LEVEL.sprites || [];
        console.log(`Buscando en ${todosLosSprites.length} sprites totales...`);

        const gemSprites = todosLosSprites.filter(s => {
            // Añadimos comprobación de existencia para evitar errores si s.type es null
            const type = (s.type || "").toLowerCase();
            const name = (s.name || "").toLowerCase();
            return type.includes('gema') || name.includes('gema');
        });

        gemSprites.forEach((s, index) => {
            this.gems.push({
                id: `gem-${index}`,
                x: s.x,
                y: s.y,
                width: s.width || 15,
                height: s.height || 14,
                type: "Gema", // ¡Añade esto para que Flutter lo reconozca!
                animationId: s.animationId,
                visible: true
            });
        });

        console.log(`Gemas detectadas en sprites: ${this.gems.length}`);
    }
    ...
    getSnapshotState() {
        ...
        return {
            ...
            gems: this.gems.filter(gem => gem.visible).map((gem) => ({
                id: gem.id,
                type: gem.type || "Gema", // Si gem.type es undefined, enviamos "Gema"
                x: round2(gem.x),
                y: round2(gem.y),
                width: gem.width,
                height: gem.height,
    ...
    serializeGameplayPlayer(player) {
        return {
            ...
            isShooting: player.isShooting || false,
    ...
    getVisibleGems() {
        return this.gems
            ...
            .map((gem) => ({
                ...
                height: gem.height,
                value: gem.value,
                animationId: gem.animationI
            }));
    }
    // Crea esta función nueva para los tanques
    getVisibleTanks() {
        return this.tanks.map((tank) => ({
            ...
            width: tank.width,
            height: tank.height,
            animationId: tank.animationId // <--- VITAL para Flutter
        }));
    }

    // Actualiza el GameplayState para incluirlos
    getGameplayState() {
        const players = Array.from(this.players.values()).sort(comparePlayers);
        return {
            ...this.getGameplayStateBase(players),
    ...
    resetPlayerForMatch(player, index) {
        ...
        player.velocityY = 0;
        player.score = 100;
        player.isDead = false; // <--- Asegúrate de que no empiece muerto
        player.isShooting = false;
    ...
    collectTouchedGems(player) {
        for (const gem of this.gems) {
            ...
            if (rectsOverlap(
                ...
                player.health = Math.min((player.health || 0) + 20, 100);

                // 1. Desactivamos la gema
                gem.visible = false;

                // 2. Programamos que vuelva a aparecer en 5 segundos (5000ms)
                setTimeout(() => {
                    gem.visible = true;
                    // Opcional: emitir un sonido o log
                    console.log("Una gema ha reaparecido");
                }, 5000);

                console.log(`${player.name} recogió gema. Vida: ${player.health}`);
            }
        }
    }
    ...
    /*spawnGems() {
        ...
        // ELIMINAMOS el shuffle para que respeten el orden del JSON
        const cells = LEVEL.gemCells || [];

        // Cola de tipos de gemas (puedes ajustar cuántas quieres de cada una)
        const spawnQueue = [];
        ...
        for (let i = 0; i < cells.length; i++) {
            const cell = cells[i];
            // Si hay tipos en la cola usamos esos, si no, gema azul por defecto
            const type = spawnQueue[i] || 'blue';
    ...
    playerOverlapsAnyZone(player, zoneIndices) {
        ...
        if (!zoneIndices || zoneIndices.length === 0) return false;

        const playerRect = rectAt(player.x, player.y, player.width, player.height);
        for (const zoneIndex of zoneIndices) {
            const zone = this.zoneRuntimeStates[zoneIndex];
            if (zone && rectsOverlap(playerRect, zone)) {
                return true;
            }
        }
        return false;
    ...
    wouldCollideBlocked(player, x, y) {
        // 1. Crear el rectángulo del jugador en la posición deseada (x, y)
        const playerRect = rectAt(x, y, player.width || 20, player.height || 20);

        // 2. Revisar solo las zonas que marcamos como "bloqueadas" (muros, plantas, etc.)
        for (const zoneIndex of this.wallZoneIndices) {
            // Acceder a los datos que cargaste en el JSON
            const zone = LEVEL.zones[zoneIndex];

            if (zone) {
                // 3. Crear el rectángulo de la zona usando el width y height del JSON
                const zoneRect = rectAt(zone.x, zone.y, zone.width, zone.height);

                // 4. Comprobar si se tocan
                if (rectsOverlap(playerRect, zoneRect)) {
