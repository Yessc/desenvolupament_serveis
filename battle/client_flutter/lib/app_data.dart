...
class MultiplayerPlayer {
  ...
  factory MultiplayerPlayer.fromJson(Map<String, dynamic> json) {
    return MultiplayerPlayer(
      ...
      isShooting: json['isShooting'] as bool? ?? false,
...
class MultiplayerGem {
  ...
  factory MultiplayerGem.fromJson(Map<String, dynamic> json) {
    return MultiplayerGem(
      ...
      x: (json['x'] as num? ?? 0).toDouble(),
      y: (json['y'] as num? ?? 0).toDouble(),
      type: (json['type'] as String? ?? 'Gema').trim(),
...
class AppData extends ChangeNotifier {
  ...
  void _applySnapshotState(Map<String, dynamic> state) {
    ...
    if (state.containsKey('gems')) {
      ...
      print(
        "DEBUG: Gemas CRUDAS recibidas del servidor: ${rawGems?.length}",
      ); // <-- AÑADE ESTO

      gems = _parseGems(rawGems);
      print(
        "DEBUG: Gemas PARSEADAS correctamente: ${gems.length}",
      ); // <-- AÑADE ESTO
    }
  ...
  void _applyGameplayState(Map<String, dynamic> state) {
    ...
    if (state.containsKey('gems') && state['gems'] != null) {
      try {
        final List<dynamic> rawGems = state['gems'] as List<dynamic>;
        // Usamos el parser asegurándonos de que no devuelva null
        final List<MultiplayerGem> parsedGems = _parseGems(rawGems);

        // Solo actualizamos si hemos conseguido parsear algo o si el servidor dice explícitamente que está vacío
        gems = parsedGems;

        /*if (kDebugMode) {
          print("DEBUG: Gemas procesadas: ${gems.length}");
        }*/
      } catch (e) {
        if (kDebugMode) {
          print("Error parseando gemas en GameplayState: $e");
        }
      }
    }
  ...
  void sendShootAction() {
    final player = localPlayer; // Creamos una copia local

    // Ahora Dart sabe que si 'player' no es null aquí,
    // seguirá sin ser null en la siguiente línea.
    if (player != null && player.isDead) {
      print("Acción bloqueada: Estás muerto");
      return;
    }
    ...
    players = ids
        .map((String id) {
          ...
          return MultiplayerPlayer(
            ...
            health: (dynamicData?.health ?? 100).toDouble(),
            ...
            isShooting: dynamicData?.isShooting ?? false,
            isDead: dynamicData?.isDead ?? false,
          );
        })
        .cast<MultiplayerPlayer>()
        .toList(growable: false);
  }
