...
class PlayScreen extends ScreenAdapter {
  ...
  void render(double delta) {
    ...
    if (Gdx.input.isKeyJustPressed(Input.keys.space)) {
      appData
          .sendShootAction(); // Llamamos a la función que crearemos en AppData
    }
    ...
    if (appData.localPlayer != null) {
      //print("Mi puntuación actual: ${appData.localPlayer!.score}");
    }
  }
  ...
  void _renderPlayers(
    ...
    // Ordenamos para que el jugador local siempre se dibuje encima de los demás
    ...
    for (final MultiplayerPlayer player in orderedPlayers) {
      // 1. Obtenemos el frame. Si tu _playerFrameFor ya maneja 'isDead',
      // devolverá la animación de la explosión automáticamente.
      final _AnimatedSpriteFrame frame = _playerFrameFor(player);
      final bool isLocalPlayer = player.id == localPlayerId;

      // Manejo de la transparencia para jugadores que no somos nosotros
      if (!isLocalPlayer) {
      ...
      // 2. Dibujamos al jugador (ya sea vivo, disparando o explotando)
      _drawAnimatedSprite(
        batch,
        frame: frame,
        worldX: player.x,
      ...
      // 3. Si somos nosotros Y NO ESTAMOS MUERTOS, calculamos el resaltado (anillo azul)
      if (isLocalPlayer && !player.isDead) {
    ...
    // Restauramos el color original del batch al terminar
    if (usingRemotePlayerOpacity) {
      batch.setColor(previousBatchColor);
    }
  }
  ...
  void _renderGems(SpriteBatch batch, List<MultiplayerGem> gems) {
    for (final MultiplayerGem gem in gems) {
      final String type = normalize(gem.type);

      // Buscamos el template ('gema' o 'tank_r')
      final LevelSprite? template =
          gemTemplateByType[type] ?? gemTemplateByType['gema'];

      if (template != null) {
        // USAMOS TU FUNCIÓN EXISTENTE: _frameFromTemplate
        // Ella ya usa 'elapsedSeconds' internamente para calcular la animación
  ...
  Map<String, LevelSprite> _buildGemTemplates(LevelData data) {
    ...
    for (final LevelSprite sprite in data.sprites.iterable()) {
      ...
      if (type == 'gema') {
        templates['gema'] = sprite;
      } else if (type == 'tank_r') {
        templates['tank_r'] = sprite;
      }
    }
    return templates;
  }
  ...
  _AnimatedSpriteFrame _playerFrameFor(MultiplayerPlayer player) {
    ...
    if (player.isDead) {
      animationName = 'player_blue_dead';
    }
    // 2. SEGUNDA PRIORIDAD: DISPARO
    else if (player.isShooting) {
      animationName = 'player_blue_dispara';
    } else {
      switch (facing) {
        case 'left':
